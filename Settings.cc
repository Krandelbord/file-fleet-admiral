...
Settings::Settings() {
    m_key_file = new Glib::KeyFile();
	
	const gchar *konf_rootdir = g_get_user_config_dir();
	m_file_name = Glib::ustring(g_build_filename(konf_rootdir, "gfm.cfg", NULL));
	m_key_file->load_from_file(m_file_name, Glib::KEY_FILE_KEEP_COMMENTS);
}

int Settings::readInteger(const Glib::ustring &name) const {
    return m_key_file->get_integer(MAIN_CATEGORY_NAME, name.c_str());
}
...
void Settings::saveInteger(const Glib::ustring &name, int value) {
	return m_key_file->set_integer(MAIN_CATEGORY_NAME, name, value);
}
...
**/
    ...
    **/
    m_key_file->save_to_file(m_file_name);
}
